...
if ($userSources.Count -eq 0) {
    if (Test-Path $localSourceC) {
        ...
        throw "Benchmark source not found: provide -Sources or add $localSourceC / $localSourceAsm"
    }
}
...
$linkerScript = Join-Path $benchmarkDir "benchmark.ld"
$base = "bench_" + $Name
...
Write-Host "Building benchmark: $Name"
Write-Host "GCC: $gcc"
Write-Host "Sources:"
$userSources | ForEach-Object { Write-Host ("  " + $_) }
